...
function Edit_worker(props:any) {

  debugger;

  
    const [name, set_name] = useState(props.worker.name);
    const [user_image, set_user_image] = useState(props.worker.user_image);
    const [last_name, set_last_name] = useState(props.worker.last_name);
    const [gender, set_gender] = useState(props.worker.gender);

    const [address, set_address] = useState(props.worker.address);
    const [t_z, set_t_z] = useState(props.worker.t_z);
    const [email, set_email] = useState(props.worker.email);
    const [phone, set_phone] = useState(props.worker.phone);
    const [b_date, set_b_date] = useState(props.worker.b_date?props.worker.b_date.split('T')[0]:"");
    const [snif, set_snif] = useState(props.worker.snif.split('|'));
    const [role, set_role] = useState(props.worker.role);
    
    const [w_level, set_w_level] = useState(props.worker.w_level);
    const [salery_hour, set_salery_hour] = useState(props.worker.salery_hour);
    const [access_app, set_access_app] = useState<any>(props.worker.access_app=='true'?true:false);
    const [access_manage, set_access_manage] = useState<any>(props.worker.access_manage=='true'?true:false);
    const [access_shifts, set_access_shifts] = useState<any>(props.worker.access_shifts=='true'?true:false);
    const [access_reports, set_access_reports] = useState<any>(props.worker.access_reports=='true'?true:false);

    const [doc_1, set_doc_1] = useState<any>(props.worker.doc_1);
    const [doc_2, set_doc_2] = useState<any>(props.worker.doc_2);
    const [doc_3, set_doc_3] = useState<any>(props.worker.doc_3);
    const [doc_1_date, set_doc_1_date] = useState<any>(props.worker.doc_1_date?props.worker.doc_1_date.split('T')[0]:"");
    const [doc_2_date, set_doc_2_date] = useState<any>(props.worker.doc_2_date?props.worker.doc_2_date.split('T')[0]:"");
    const [doc_3_date, set_doc_3_date] = useState<any>(props.worker.doc_3_date?props.worker.doc_3_date.split('T')[0]:"");

    const [salery_type, set_salery_type] = useState<any>(props.worker.salery_type);
    const [salery_number, set_salery_number] = useState(props.worker.salery_number);
    const [work_start, set_work_start] = useState(props.worker.work_start?props.worker.work_start.split('T')[0]:'');
    const [work_end, set_work_end] = useState(props.worker.work_end?props.worker.work_end.split('T')[0]:'');
    ...
    function update_worker(){
      ...
      payload.append("user_image",user_image);
      ...
      axios({
        method: 'post',
        url: globalThis.app.current+'/update_worker',
        data: payload,
        headers: { "Content-Type": "multipart/form-data" }, 
      }).then(() => {
    ...
    async function upload_user_img(files:any){
        ...
        if (files[0] && files[0]) {
            ...
            await axios({
              ...
              set_user_image(res.data.fileName);
    ...
    // const [status, set_status] = useState(props.worker.status);
    // const [name, set_name] = useState(props.worker.name);
    // const [role, set_role] = useState(props.worker.role);
    // const [snif, set_snif] = useState(props.worker.snif);
  ...
  return (
    <>
      <div className='worker_popup_cont' >
        <img src={close} className='close_icon' onClick={()=>props.set_edit_worker_popup(false)}/>
        <div className='worker_popup_header'>
            {/* <img src={user_image?globalThis.app.current+'/'+user_image:p_2_p_b} className='user_b user_b_img'/>     */}

               {user_image==''?<div className='user_img_cont' onClick={focusInput4}>
              <img src={camera} />
              <div>הוסף תמונה</div>
              <input type="file" onChange={(e)=>upload_user_img(e.target.files)} ref={inputElement4} className='file_input' />

            </div>:<><img src={globalThis.app.current+'/'+user_image} className='user_img_cont' onClick={focusInput4}/>
            <input type="file" onChange={(e)=>upload_user_img(e.target.files)} ref={inputElement4} className='file_input' />
            </>}

            <div className='user_b_text_cont'>
                <div className='user_b_text_main_tachmoshet'>עריכת עובד</div>
      ...
      <div className='worker_info_cont'>
                ...
                <div className='tachmoshet_info_type_cont_main_row'>
                    ...
                    <div className=' tachmoshet_info_type_cont_main'>
                        ...
                        <input type="date" className='tachmoshet_info_type_cont tachmoshet_info_type_row_date' placeholder='תאריך לידה' value={b_date} onChange={(e)=>set_b_date(e.target.value)}/>
                    </div>
                </div>
              ...
              <div className='tachmoshet_info_type_cont_main_row'>
                ...
                <div className='tachmoshet_info_type_cont_main_row'>
                    <div className=' tachmoshet_info_type_cont_main'>
                        ...
                        <input type="date" className='tachmoshet_info_type_cont tachmoshet_info_type_row_date salery_worker_input' placeholder='הזן מספר מזהה בשכר' value={work_start} onChange={(e)=>set_work_start(e.target.value)}/>
                    </div>
                </div>
                ...
                <div className='tachmoshet_info_type_cont_main_row'>
                    <div className=' tachmoshet_info_type_cont_main'>
                        ...
                        <input type="date" className='tachmoshet_info_type_cont tachmoshet_info_type_row_date salery_worker_input' placeholder='הזן מספר מזהה בשכר' value={work_end} onChange={(e)=>set_work_end(e.target.value)}/>
                    </div>
                </div>
            ...
            <div className='new_worker_docs_needed'>
                ...
                <div className='tachmoshet_info_type_cont_main_row_h'>
                    <div className=' tachmoshet_info_type_cont_main'>
                      ...
                      <input type="date"  value={doc_1_date} onChange={(e)=>set_doc_1_date(e.target.value)} className='tachmoshet_info_type_cont tachmoshet_info_type_row_date tachmoshet_info_type_row_date_s' />
                    ...
                    <div className=' tachmoshet_info_type_cont_main'>
                      ...
                      <input type="date"  value={doc_2_date} onChange={(e)=>set_doc_2_date(e.target.value)} className='tachmoshet_info_type_cont tachmoshet_info_type_row_date tachmoshet_info_type_row_date_s' />
                    ...
                    <div className=' tachmoshet_info_type_cont_main'>
                      ...
                      <input type="date"  value={doc_3_date} onChange={(e)=>set_doc_3_date(e.target.value)} className='tachmoshet_info_type_cont tachmoshet_info_type_row_date tachmoshet_info_type_row_date_s' />
      ...
      {show_eq_popup ?<div className='eq_popup_cont'>
        ...
        <div className='save_t_btn_cont'>
            <div className='save_t_btn' onClick={()=>update_worker()}>עדכן עובד</div>
            <div className='cancel_t_btn'  onClick={()=>props.set_edit_worker_popup(false)}>ביטול</div>
        </div>
